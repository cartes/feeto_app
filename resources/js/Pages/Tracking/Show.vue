            ...
            <div class="max-w-md mx-auto px-6 py-4 flex items-center justify-between">
                <div class="flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <p class="text-[10px] uppercase tracking-widest text-gray-400 font-bold">Seguimiento en línea</p>
                    </div>
                </div>
            </div>
        </header>
        ...
        <main class="w-full max-w-md mx-auto px-6 mt-8 flex-1 flex flex-col">
            ...
            <div class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 mb-8 w-full">
                <div class="flex justify-between items-start mb-4 pb-4 border-b border-gray-50">
                    <div>
                        <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">Orden de Trabajo</p>
                    ...
                    <div class="bg-gray-50 border border-gray-200 px-3 py-1.5 rounded-xl shadow-inner text-center">
                        ...
                        <p class="text-lg font-mono font-black tracking-widest text-[#F9A826] leading-none mt-0.5">{{ workOrder.vehicle?.plate }}</p>
                    </div>
                </div>
                <div>
                    <p class="font-bold text-gray-800 text-sm uppercase">{{ workOrder.vehicle?.brand }} {{ workOrder.vehicle?.model }}</p>
                    <p class="text-xs text-gray-500 mt-0.5 font-medium">Ingresado el {{ formatDate(workOrder.created_at) }}</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 w-full mb-8 relative">
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div class="relative flex-shrink-0">
                                <template v-if="index < currentIndex">
                                    <div class="w-6 h-6 rounded-full bg-emerald-500 border-4 border-white shadow flex items-center justify-center">
                                        <svg class="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" /></svg>
                                    </div>
                                </template>
                                <template v-else-if="index === currentIndex">
                                    <div class="w-6 h-6 rounded-full bg-white border-4 border-[#F9A826] shadow ring-4 ring-[#F9A826]/20 flex items-center justify-center">
                ...
                <div v-if="workOrder.observations" class="mt-8 p-4 bg-amber-50 rounded-2xl border border-amber-100/50">
                    <p class="text-[10px] font-black text-amber-500 uppercase tracking-widest mb-1.5 flex items-center gap-1">
            ...
            <!-- Boton CTA WhatsApp -->
            <a :href="wsLink" target="_blank" class="w-full bg-[#25D366] hover:bg-[#128C7E] text-white px-6 py-4 rounded-full font-black text-sm uppercase tracking-wider flex items-center justify-center gap-3 shadow-[0_10px_20px_rgba(37,211,102,0.3)] transition-all active:scale-95">
