...
const subtotalAmount = computed(() => Number(props.quote.subtotal_amount ?? 0));
...
watch(() => props.quote, (newQuote) => {
    if (!newQuote) return;
    taxForm.apply_tax = Boolean(newQuote.apply_tax ?? true);
    taxForm.tax_rate = Number(newQuote.tax_rate ?? props.defaultTaxRate);
}, { deep: true });

const isEditingTax = ref(false);

const submitTaxUpdate = () => {
    taxForm.patch(route('quotes.tax.update', { quote: props.quote.id }), {
        preserveScroll: true,
        onSuccess: () => {
                            ...
                            <table class="w-full">
                                ...
                                <tfoot>
                                    ...
                                    <tr class="border-t-2 border-gray-100 bg-gray-50/80">
                                        ...
                                        <td class="px-4 py-4 text-right">
                                            <span class="text-lg font-black tabular-nums text-gray-900">{{ formatCurrency(finalTotalAmount) }}</span>
                                            <span v-if="formatUf(finalTotalAmount)" class="block text-[10px] font-semibold tabular-nums text-gray-400">UF {{ formatUf(finalTotalAmount) }}</span>
                                        </td>
                                        <td></td>
                                    </tr>
                                </tfoot>
