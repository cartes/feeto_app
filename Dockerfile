...
RUN --mount=type=cache,target=/tmp/composer-cache \
    ...
    sh -eu -c 'if [ -s /run/secrets/composer_auth ]; then secret="$(cat /run/secrets/composer_auth | tr -d "\r\n" | sed -e "s/^[[:space:]]*//" -e "s/[[:space:]]*$//")"; case "$secret" in \{*) export COMPOSER_AUTH="$secret" ;; *) export COMPOSER_AUTH="{\"github-oauth\": {\"github.com\": \"$secret\"}}" ;; esac; fi; composer install --no-dev --no-interaction --no-progress --prefer-dist --optimize-autoloader --no-scripts --ignore-platform-reqs -vvv'
